...
## 📢 More Information: Employee Approval & Login Post-Recruitment

Once the recruitment process is complete, the new employee will appear in the **Employee Master** section of HR Support, highlighted in red for easy identification.

**📝 Steps for Employee Approval and Login Enablement:**

1. **👥 User Creation:**
   - An authorized user with access to Employee Master must initiate user creation for the new employee.
   - Select the appropriate role for the employee and confirm by clicking **OK**.

2. **🔑 Contract Code Assignment:**
   - Click on **Add Contract Code** for the employee.
   - Enter the contract code and submit the form.

3. **🖥️ System Visibility:**
   - Upon successful user creation, the employee will be listed in the **User Creation** section under the Root Master menu in the Institute module.
   - The employee can now log in to the system.

4. **✅ Final Approval:**
   - Once the contract code is assigned, authorized personnel under the **New Staff Approver** submenu (within the Approver menu) can view the employee entry.
   - Approve the employee contract to finalize the process.

5. **🎉 Completion:**
   - After approval, the employee's entry in Employee Master will no longer be highlighted in red, indicating successful onboarding and activation.
